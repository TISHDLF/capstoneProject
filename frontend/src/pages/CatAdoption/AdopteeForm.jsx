...
const AdopteeForm = () => {
  ...
  const generateAdopteePDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < Math.ceil(imgHeight / pageHeight); i++) {
        if (i > 0) pdf.addPage();
      ...
      const response = await axios.post(
        "http://localhost:5000/adopt/apply",
        formData,
        {
    ...
    const fetchCatAndImages = async () => {
      try {
        ...
        const imagesRes = await fetch(
          `http://localhost:5000/cats/${cat_id}/images`
        );
    ...
    <div className="flex flex-col min-h-screen md:pb-10 pb-24">
      ...
      <div className="md:grid md:grid-cols-[80%_20%] h-full">
        <div className="flex flex-col md:pl-50 md:p-10 p-4">
          <div className="relative md:grid md:grid-cols-[30%_70%] flex flex-col bg-[#FFF] md:w-[1000px] w-full rounded-[25px]">
            {/* Left Side - Instructions */}
            <div className="flex flex-col gap-6 md:gap-10 p-4 md:p-0">
              <Link
                onClick={handleGoBack}
                className="flex flex-row items-center gap-3 bg-[#DC8801] rounded-[15px] p-3 w-fit hover:bg-[#ffa71a] active:bg-[#DC8801]"
              >
                ...
                <label className="font-bold text-[#FFF] text-sm md:text-base">
                  Back
                </label>
              </Link>
              <div className="flex flex-row justify-between items-center p-3 bg-[#FDF5D8] md:rounded-tr-[20px] md:rounded-br-[20px] rounded-[15px] shadow-md">
            ...
            <form
              ref={printRef}
              className="flex flex-col items-start p-4 md:p-10 gap-6 md:gap-10"
              onSubmit={(e) => {
                e.preventDefault();
                generateAdopteePDF();
              }}
            >
              <div className="flex md:flex-row flex-col gap-4 w-full md:justify-between">
                <div className="flex flex-col">
                  <label className="text-sm md:text-base">
                    1. You will be adopting:
                  </label>
                  <label className="font-bold pl-4 text-sm md:text-base">
                    {cat.name}
                  </label>
                </div>
                <div className="border-dashed border-2 border-[#B5C04A] rounded-[18px] p-2 self-center">
                  <div className="w-full md:w-[250px] h-auto rounded-[15px] overflow-hidden">
                    <img
                      src={imageSrc}
                      alt={`${cat.name} photo`}
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 w-full gap-2">
                  <label
                    htmlFor="radioFBPage"
                    className="flex items-center gap-2 p-2 border-1 border-[#252525] rounded-[8px] text-xs md:text-sm"
                  >
                    <input
                      type="radio"
                      id="radioFBPage"
                  ...
                  <label
                    htmlFor="other"
                    className="flex gap-2 p-2 border-1 border-[#252525] rounded-[8px] text-xs md:text-sm"
                  >
                    <input
                      type="radio"
                      name="adaptionOpportunity"
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-col">
                  ...
                  <label className="italic text-xs md:text-[14px] text-[#828282]">
                    (You may opt to blur sensitive data like Social Security
                    info etc.)
                  </label>
                </div>
                <div className="flex md:flex-row flex-col items-center gap-3">
                  <div className="flex items-center justify-center w-full md:w-[200px] h-[150px] md:h-full border-dashed border-2 border-[#898989] p-2 object-cover rounded-[15px]">
                    <img
                  ...
                  <div className="flex flex-col w-full md:w-auto">
                    <label
                      htmlFor="idUpload"
                      className="bg-[#DC8801] p-3 rounded-[10px] text-[#FFF] text-center cursor-pointer text-sm md:text-base"
                    >
                      Attach ID Photo
                      <input
                        type="file"
                    ...
                    <label className="italic text-[#828282] text-xs md:text-[14px] mt-2">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm md:text-base">
                  4. Have you raised a cat before?
                </label>
                <label
                  htmlFor="raisedYes"
                  className="flex items-center gap-2 p-2 border-1 border-[#252525] rounded-[8px] text-xs md:text-sm"
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col">
                  ...
                  <label className="italic text-xs md:text-[14px] text-[#7d7d7d]">
                ...
                <input
                  ...
                  className="flex items-center p-2 border-1 border-[#252525] rounded-[8px] text-sm md:text-base"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-col">
                  ...
                  <label className="italic text-xs md:text-[14px] leading-tight text-[#898989]">
              ...
              <button className="flex justify-center w-full text-center bg-[#B5C04A] text-[#FFF] font-bold p-3 rounded-[15px] hover:bg-[#CFDA34] active:bg-[#B5C04A] text-sm md:text-base">
                Submit
              </button>
            </form>
          </div>
