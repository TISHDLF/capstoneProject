    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/admin/manage/non_admin`
        );
  ...
  const assignAdmin = async (event, user_id) => {
    ...
    try {
      const response = await axios.patch(
        "http://localhost:5000/admin/manage/update_admin",
        {
          user_id,
        }
      );
  ...
  return (
    <div className="absolute bottom-0 flex flex-col w-full min-h-[300px] bg-[#FFF] gap-5 p-5 pb-10 rounded-tr-[15px] rounded-tl-[15px] m-10 border-t-2 border-t-[#2F2F2F] border-r-2 border-r-[#2F2F2F] border-l-2 border-l-[#2F2F2F]">
      ...
      <div className="flex w-full justify-start gap-2 ">
        <input
          type="search"
          placeholder="Search for a canditate"
          value={searchInput}
        ...
        <button
          type="button"
          className="cursor-pointer p-2 pl-4 pr-4 rounded-[25px] bg-[#DC8801] active:bg-[#feaf2f] text-[#FFF] font-bold"
        >
          Search
        </button>
      </div>
