...
const UpdateRole = () => {
    ...
    const fetchUser = async (user_id) => {
      try {
        const response = await axios.get(
          `http://localhost:5000/manage/role/${user_id}`
        );
  ...
  const handleRoleUpdate = async (event) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:5000/manage/update/${user_id}`,
        {
  ...
  return (
    <form
      ...
      className="absolute bottom-0 flex flex-col w-full min-h-[250px] gap-5 bg-[#FFF] p-5 rounded-tr-[15px] rounded-tl-[15px] m-10 border-t-2 border-t-[#2F2F2F] border-r-2 border-r-[#2F2F2F] border-l-2 border-l-[#2F2F2F]"
    >
      <div className="flex flex-col w-full gap-3 pb-3 border-b-1 border-b-[#CCCCCC]">
        <div className="flex justify-between w-full">
          <label className="self-start text-[24px] text-[#2F2F2F] font-bold">
            Update Role
          </label>
          <Link to="/adminlist" className="cursor-pointer">
            Close
          </Link>
        </div>
      </div>
        ...
        <div className="flex justify-between w-full gap-4">
          ...
          <div className="flex flex-col w-full justify-start">
            <label className="text-[#595959] text-[14px]">
              {isEditingSelf
                ? "Role: Can't update role if currently logged In"
                : "Role"}
            </label>
            <select
