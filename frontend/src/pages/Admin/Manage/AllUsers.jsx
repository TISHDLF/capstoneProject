...
const AllUsers = () => {
  ...
  useEffect(() => {
    const fetchusers = async () => {
      try {
        const response = await axios.get('http://localhost:5000/manage/users')
            ...
            <div className='flex flex-col items-center p-10 min-h-screen gap-5 mx-auto'>
                ...
                <div className='flex flex-row justify-between w-full'>
                  <form className='flex w-full gap-2'>
                    <input type="search" placeholder='Search' className='bg-[#FFF] p-2 w-full border-1 border-[#595959] rounded-[15px]'/>
