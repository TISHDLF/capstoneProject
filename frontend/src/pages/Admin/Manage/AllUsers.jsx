...
const AllUsers = () => {
  ...
  const handleDelete = async (user_id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/admin/manage/user/${user_id}`);
      setUsers((prev) => prev.filter((u) => u.user_id !== user_id)); // update UI
    } catch (err) {
        ...
        <div className="flex flex-col items-center p-10 min-h-screen gap-5 mx-auto">
          ...
          <table className="flex flex-col gap-2 w-full">
            <thead className="flex w-full">
              <tr className="grid grid-cols-[10%_20%_30%_20%_10%_10%] w-full place-items-start bg-[#DC8801] text-[#FFF] rounded-[10px] p-3">
                <th>User ID</th>
                <th>Name</th>
                <th>Email</th>
                <th>Role</th>
            ...
            <tbody className="flex flex-col w-full gap-1 overflow-y-scroll min-h-[550px]">
              {filteredItems.map((user) => (
                <tr
                  key={user.user_id}
                  className="grid grid-cols-[10%_20%_30%_20%_10%_10%] w-full place-items-center justify-items-start bg-[#FFF] text-[#2F2F2F] rounded-[10px] p-3 border-b-1 border-b-[#595959]"
                >
                  <td>{user.user_id}</td>
                  ...
                  <td>
                    <Link
                      ...
                      className="bg-[#2F2F2F] active:bg-[#595959] text-[#FFF] p-1 pl-4 pr-4 rounded-[15px] cursor-pointer"
                    >
                      View
                    </Link>
                  </td>
                  ...
                  <td>
                    <button
                      ...
                      className="bg-lime-600 hover:bg-red-700 text-white p-1 pl-4 pr-4 rounded-[15px] cursor-pointer"
                    >
                      Delete
                    </button>
                  </td>
