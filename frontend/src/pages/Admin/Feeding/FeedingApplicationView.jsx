  ...
  const handleUpdateStatus = async (status) => {
    try {
      const response = await axios.patch(
        `http://localhost:5000/admin/form/status_update/${applicant.application_id}`,
        { status }
      );
      ...
      if (status === "Accepted") {
        setStatusMessage(
          `${applicant.firstname} ${applicant.lastname}'s application is accepted. Update successfull!`
        );
      } else if (status === "Rejected") {
        setStatusMessage(
          `${applicant.firstname} ${applicant.lastname}'s application is rejected. Update successfull!`
        );
      }
    } catch (err) {
          ...
          {applicant && (
            <div className="flex flex-col p-5 bg-[#FFF] h-auto w-full rounded-[10px] gap-2">
              ...
              {applicant.application_form && (
                <object
                  data={`http://localhost:5000/FileUploads/volunteer/${applicant.application_form}`}
                ...
                >
                  <p>
                    ...
                    <a
                      href={`http://localhost:5000/FileUploads/volunteer/${applicant.application_form}`}
                      target="_blank"
