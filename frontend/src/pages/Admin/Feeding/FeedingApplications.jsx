    ...
    const fetchApplications = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/admin/feeders/application`
        );
            ...
            <tbody className="flex flex-col w-full overflow-y-scroll min-h-[600px]">
              {applicant.map((application) => (
                ...
                >
                  ...
                  <td>
                    <Link
                      to={`/feedingapplications/feedingapplicationview/${application.application_number}`}
                      className="flex items-center gap-4 text-[#DC8801] underline font-bold hover:text-[#977655] active:text-[#DC8801]"
                    >
                      {application.application_form}
                    </Link>
                  </td>
                  <td className="bg-[#B5C04A] text-[#FFF] font-bold p-2 pl-4 pr-4 rounded-[10px]">
                    Pending
                  </td>
                </tr>
              ))}
