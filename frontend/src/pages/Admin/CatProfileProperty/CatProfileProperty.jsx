...
const CatProfileProperty = () => {
  ...
  const handleCatProfileUpdate = async (e) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:5000/cats/update/${cat_id}`,
        {
  ...
  const handleUploadImages = async (cat_id) => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:5000/cats/uploadcatimages/${cat_id}`,
        formData,
        {
          headers: {
  ...
  const fetchCatImage = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/cats/image/${cat_id}`
      );
  ...
  const handleDeleteImage = async (filename) => {
    try {
      await axios.delete(`http://localhost:5000/cats/image/${filename}`);
              ...
              <div className="flex flex-row justify-between pb-2 border-b-1 border-b-[#CFCFCF]">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    className="font-bold  text-[20px] p-2 text-[#2F2F2F] rounded-[10px] border-2 border-[#CFCFCF]"
              ...
              {!uploaderVisible && (
                <div className="flex flex-col gap-2 border-1 border-[#a3a3a3] p-3 rounded-[15px]">
                  ...
                  <div className="flex items-center justify-between">
                    <label className="text-[20px] text-[#2F2F2F] font-bold ">
                      {" "}
                      IMAGES{" "}
                    </label>
                    <button
                  ...
                  <div className="grid grid-cols-5 gap-2 w-full">
                    ...
                    {catImage.map((imageURL, index) => (
                      <div
                        key={index}
                        className="relative flex items-center max-w-[250px] h-[250px] rounded-[10px] overflow-hidden bg-[#CCCCCC] border-1 border-[#CCCCCC]"
                      >
                        <button
                          type="button"
