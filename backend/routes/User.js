...
UserRoute.get("/logged", async (req, res) => {
  ...
  try {
    ...
    const [data] = await db.query(
      "SELECT user_id, firstname, lastname, role FROM users WHERE user_id = ?",
      [userId]
    );
...
UserRoute.get("/profile", async (req, res) => {
  ...
  try {
    ...
    if (!sessionUser || !sessionUser.user_id) {
      return res
        .status(401)
        .json({ error: "Please login to submit application." });
    }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!sessionUser || !sessionUser.user_id) {
        ...
        return res
          .status(401)
          .json({ error: "User not authenticated (no session)" });
      }
  ...
  try {
    ...
    if (existing.length > 0) {
      return res.status(409).json({
        error:
          "You already have a pending application. Please wait for it to be reviewed before submitting again.",
      });
    }
    ...
    if (activeVolunteer.length > 0) {
      return res.status(409).json({
        error:
          "You already have an approved volunteer application. Please wait until it expires before applying again.",
      });
    }
