...
CatRoute.get("/catprofile/:cat_id", async (req, res) => {
  ...
  try {
    ...
    console.log("Fetched cat:", rows[0]);
    return res.json(rows[0]); // ✅ always valid JSON now
  } catch (err) {
...
CatRoute.get("/image/:cat_id", async (req, res) => {
  ...
  try {
    ...
    const formatted = images.map((img) => ({
      filename: img.image_filename,
      url: `http://localhost:5000/uploads/cats/${img.image_filename}`,
    }));
...
CatRoute.delete("/image/:filename", async (req, res) => {
  ...
  try {
    ...
    fs.unlink(filePath, async (err) => {
      ...
      // Delete DB record
      await db.query(`DELETE FROM cat_images WHERE image_filename = ?`, [
        filename,
      ]);
